...
export class Streebog {
  ...
  constructor(outputMode){
    ...
    if(!outputMode){
      this.h.fill(0x01,0,64)}
    else{
      this.h.fill(0x00,0,64)}
  ...
  hashPart(messagePart)
  {
    ...
    {
      this.h = this.compression(this.N, messagePart, this.h)
      this.N = this.addModule(this.N, this.N512)
      this.sigma = this.addModule(this.sigma, messagePart)
    }
    else
    {
      let inc = 0
      let length = messagePart.length * 8

      //Дополнение до 512 бит по алгоритму
      let tmpMessage = [64]
      tmpMessage.fill(0x00,0,64)
      tmpMessage[64 - messagePart.length - 1] =  0x01

      for(let i = 64 - messagePart.length; i < 64; i++)
  ...
  generateHash(message,outputMode)
  {
    ...
    {
      inc++;
      var tmp = message.slice(message.length - 64*(inc), message.length - (inc-1)*64)// Не уверен что правтльно отрезает
    ...
    message = message.slice(message, message.length - 64*inc)// Не уверен

    if(message.length < 64) //обработка сообщения длинной < 512 бит
    {
      var tmpMessage =[64]
      tmpMessage.fill (0x00,0,64);
      tmpMessage[64 - message.length - 1] = 0x01

      for(let i = 64 - message.length; i < 64; i++)
        tmpMessage[i] = message[i + message.length - 64]

      message = tmpMessage;
    }

    this.h = this.compression(this.N, message, this.h)
    ...
    this.N = this.addModule(this.N, NMessage)
    this.sigma = this.addModule(this.sigma, message)
    ...
    if(outputMode)
      return this.h;
    return this.h.slice(32,64)// Не уверен что правильно Arrays.copyOf(h, 32);
  }
  addModule(a,b)
  {
    var result =[64]
